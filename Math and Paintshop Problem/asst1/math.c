...
struct semaphore *finished, *mutex;
...
void adder(void * unusedpointer, unsigned long addernumber)
{
	...
	(void) unusedpointer; /* remove this line if variable is used */
  	...
  	while(flag) {
		...
		/* Wait until another thread left the critical region */
		P(mutex);
		...
		/* left the critical region and signal other threads */
		V(mutex);
  	}
	...
	mutex = sem_create("mutex", 1);
 	...
 	kprintf("Starting %d adder threads\n", NADDERS);
  	...
  	for(index = 0; index < NADDERS; index++) {
    	...
    	error = thread_fork("adder thread", NULL, adder, (unsigned long*)index, (unsigned long)index);
